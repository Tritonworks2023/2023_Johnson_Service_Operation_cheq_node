...
var catagroiesemailModel = require('./../../models/document/catagroiesemailModel');
...
router.post('/create', async function(req, res) {
  try{
        ...
        await catagroiesemailModel.create({
            user_id : req.body.user_id || "",
            user_email : req.body.user_email || "",
            user_type : req.body.user_email || "", 
...
router.post('/getlist_id', async function (req, res) {
        await catagroiesemailModel.find({user_id:req.body.user_id}, function (err, Roledetails) {
...
router.get('/getlist', async function (req, res) {
        await catagroiesemailModel.find({}, function (err, Roledetails) {
          res.json({Status:"Success",Message:"Email List", Data : Roledetails ,Code:200});
        });
});

router.post('/fetch_by_email_id', async function (req, res) {
        await catagroiesemailModel.find({email_id:req.body.email_id}, function (err, Roledetails) {
          res.json({Status:"Success",Message:"Email List", Data : Roledetails ,Code:200});
        });
});
...
router.post('/getcount', async function (req, res) {
        await catagroiesemailModel.find({user_id:req.body.user_id,message_status:"unread"}, function (err, count) {
          res.json({Status:"Success",Message:"Email count", Data : count ,Code:200});
        }).count();
});

router.get('/deletes', function (req, res) {
      catagroiesemailModel.remove({}, function (err, user) {
...
router.post('/edit', function (req, res) {
        catagroiesemailModel.findByIdAndUpdate(req.body._id, req.body, {new: true}, function (err, UpdatedDetails) {
...
router.post('/delete', function (req, res) {
      catagroiesemailModel.findByIdAndRemove(req.body.Role_id, function (err, user) {
          if (err) return res.status(500).send("There was a problem deleting the user.");
